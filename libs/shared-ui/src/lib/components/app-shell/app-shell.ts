      ...
      >
        <aside class="sidebar" [id]="sidebarId" aria-label="Painel administrativo">
          ...
          <footer class="sidebar__bottom" aria-label="Usuário autenticado">
            @if (footerLabel()) {
              <span class="ui-app-shell__footer-label">{{ footerLabel() }}</span>
            }
...
export class AppShellComponent {
  ...
  protected readonly mobileDrawerId = `${this.shellId}-drawer`;

  private readonly document = inject(DOCUMENT);
  private lastFocusedElement: HTMLElement | null = null;

  constructor() {
    // Foco volta ao botão do menu ao fechar a gaveta mobile.
    effect(() => {
