...
@Component({
  selector: 'app-step-03-modalidades',
  standalone: true,
...
export class Step03ModalidadesComponent {
  ...
  readonly allSelected = computed(() => this.store.draft().modalidades.selected.length === this.modalidades.length);
  ...
  toggleAll(checked: boolean): void {
    this.store.patchObjectSection('modalidades', { selected: checked ? this.modalidades.map((item) => item.id) : [] });
  }
