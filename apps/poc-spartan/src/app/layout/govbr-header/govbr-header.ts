    ...
    <header class="bg-govbr-primary text-govbr-pure-0 py-govbr-3 px-govbr-5 shadow-md">
      <div class="max-w-screen-xl mx-auto flex items-center justify-between">
        ...
        <div class="flex items-center gap-govbr-4">
          ...
          <!-- Busca -->
          <button class="hidden md:flex items-center justify-center w-8 h-8 rounded-govbr-pill
                         hover:bg-govbr-primary-hover transition-colors"
                  aria-label="Buscar">
          ...
          <!-- Login/avatar -->
          <button class="flex items-center gap-govbr-2 px-govbr-3 py-govbr-1
                         border border-govbr-pure-0/40 rounded-govbr-pill
                         hover:bg-govbr-primary-hover transition-colors text-govbr-sm font-medium"
                  aria-label="Entrar">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                 fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      @if (mobileMenuAberto()) {
        <nav class="md:hidden mt-govbr-3 pt-govbr-3 border-t border-govbr-pure-0/20 flex flex-col gap-govbr-3">
          ...
          <button class="flex items-center gap-govbr-2 text-govbr-sm font-medium py-govbr-1"
                  aria-label="Buscar">
