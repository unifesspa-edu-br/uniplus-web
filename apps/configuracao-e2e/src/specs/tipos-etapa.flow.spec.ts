...
test.describe('Tipo de etapa — CRUD', () => {
  ...
  test('o código é somente leitura na edição, e o que o tipo admite é editável', async ({
    ...
    await page.getByRole('button', { name: `Editar tipo de etapa ${analiseDocumental.codigo}`, exact: true }).click();
  ...
  test('inativa um tipo após confirmação', async ({ page }) => {
    ...
    await page.getByRole('button', { name: `Inativar tipo de etapa ${analiseDocumental.codigo}`, exact: true }).click();
